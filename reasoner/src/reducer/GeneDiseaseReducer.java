...
public class GeneDiseaseReducer extends Reducer<Text, Text, NullWritable, Text> {
	...
	protected void reduce(Text key, Iterable<Text> values,
	    ...
	    int rule=Integer.parseInt(keyArr[0]);
		
		for(Text value : values) {
			Triple triple = TripleTool.parseLineToTriple(value.toString());
			index = OWLRuleChainUtil.getIndexByPredicate(triple.getPredicate(),rule);
			if(index==-1)
				return;
			current = index % 2;

			//if the length of rule is odd, and its predicate order is the last. write the triple
			if((index+1)==OWLRuleChainUtil.getPredicateNum(rule)&&current==0){
		...
		String outputPredicate = (current == 0) ? OWLRuleChainUtil.getPredicateByIndex(index,rule)+"$$"+OWLRuleChainUtil
				.getPredicateByIndex(index+1,rule) : OWLRuleChainUtil
				.getPredicateByIndex(index-1,rule)+"$$"+OWLRuleChainUtil.getPredicateByIndex(index,rule);
